...
function getRebalanceValueStatsCVL(IStrategy.RebalanceParams input) returns LMPStrategy.RebalanceValueStats {
    ...
    uint256 swapCost;
    uint256 slippage;
    LMPStrategy.RebalanceValueStats tmp = setValueStatsSlippage(e, inPrice, outPrice, inEthValue, outEthValue, swapCost, slippageCVL);
    return tmp;
}
...
rule notExceedingMaxSlippage(
    ...
    require valueStats.slippage - getmaxNormalOperationSlippage(e) > 0;
    slippageCVL = valueStats.slippage;
...
rule correctPause() {
    env e;
    uint40 lastPausedTimestamp = getlastPausedTimestamp(e);
    uint16 pauseRebalancePeriodInDays = getpauseRebalancePeriodInDays(e);
    require lastPausedTimestamp != 0;
    require e.block.timestamp <= assert_uint256(assert_uint256(lastPausedTimestamp) + assert_uint256(pauseRebalancePeriodInDays * 24 * 3600));
...
//     uint40 lastPausedTimestamp = getlastPausedTimestamp(e);
//     uint16 pauseRebalancePeriodInDays = getpauseRebalancePeriodInDays(e);
//     require lastPausedTimestamp == 0 || e.block.timestamp > assert_uint256(assert_uint256(lastPausedTimestamp) + assert_uint256(pauseRebalancePeriodInDays * 24 * 3600));
//     bool isPaused = paused(e);
//     assert !isPaused;
// }

rule navUpdateNoUpdateTimeWhenPaused(
    uint256 navPerShare
) {
    env e;
    
    require paused(e);
    uint40 prevPausedTime = getlastPausedTimestamp(e);
    navUpdate(e, navPerShare);
    uint40 afterPausedTime = getlastPausedTimestamp(e);

    assert prevPausedTime == afterPausedTime;
}
