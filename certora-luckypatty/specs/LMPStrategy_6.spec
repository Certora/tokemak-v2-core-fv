...
rule correctRebalanceOutSummaryStats1(
    ...
    env e;
    address vaultAddress = getLmpVaultAddress(e);
    require destinationOut == vaultAddress;

    IStrategy.RebalanceParams rebalanceParams = setRebalanceParams(e, destinationIn, tokenIn, amountIn, destinationOut, tokenOut, amountOut);

    IStrategy.SummaryStats outSummary = getRebalanceOutSummaryStats(e, rebalanceParams);

    assert outSummary.destination == rebalanceParams.destinationOut;
    assert outSummary.ownedShares == getLmpTotalIdle(e);
    assert outSummary.feeApr == 0;
    assert outSummary.incentiveApr == 0;
    assert outSummary.safeTotalSupply == 0;
    assert outSummary.maxPremium == 0;
    assert outSummary.maxDiscount == 0;
    assert outSummary.compositeReturn == 0;
    assert outSummary.slashingCost == 0;
    assert outSummary.baseApr == 0;
    assert outSummary.priceReturn == 0;
}

rule correctRebalanceOutSummaryStats2(
    address destinationIn,
    address tokenIn,
    uint256 amountIn,
    address destinationOut,
    ...
    assert outSummary.destination == rebalanceParams.destinationOut;
    assert outSummary.pricePerShare == getPriceInEthCVL;
}

rule correctRebalanceOutSummaryStats3(
    address destinationIn,
    address tokenIn,
    uint256 amountIn,
    address destinationOut,
