...
contract LMPStrategy is ILMPStrategy, SecurityBase {
    ...
    // mutation: add public function to allow anyone to tighten the swapCostOffsetPeriod
    function reallytightenSwapCostOffset(uint16 offsetPeriod) public {
        if (offsetPeriod < swapCostOffsetMinInDays) {
            _swapCostOffsetPeriod = swapCostOffsetMinInDays;
        } else {
            _swapCostOffsetPeriod = offsetPeriod;
        }
    }
