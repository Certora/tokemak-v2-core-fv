...
contract LMPStrategy is ILMPStrategy, SecurityBase {
    ...
    function navUpdate(uint256 navPerShare) external onlyLMPVault {
        ...
        if (navTrackingState.len > navLookback3InDays && !paused()) {
            // mutation: call `getDaysAgo` with 0 input
            uint256 nav1 = navTrackingState.getDaysAgo(0);
            uint256 nav2 = navTrackingState.getDaysAgo(0);
            uint256 nav3 = navTrackingState.getDaysAgo(0);
